...
export const usePerformanceMetrics = (tradeHistory) => {
  ...
  const calculateMetrics = useCallback(() => {
    ...
    let totalLossAmountInternal = 0
    let totalRealizedRRSUM = 0
    let realizedRRCount = 0
    ...
    tradeHistory.forEach((trade) => {
      ...
      const strategy = trade.strategyUsed || 'Unknown'
      ...
      if (pnl > 0) {
        ...
        losingTrades++
        totalLossAmountInternal += pnl
      } else {
        breakEvenTrades++
      }

      if (typeof trade.riskReward === 'number' && !isNaN(trade.riskReward)) {
        totalRealizedRRSUM += trade.riskReward
        realizedRRCount++
      }
      ...
      if (
        ...
        let plannedReward = 0
        if (direction === 'Long') {
          plannedRisk = entry - stopLoss
          plannedReward = target - entry
        } else if (direction === 'Short') {
          plannedRisk = stopLoss - entry
          plannedReward = entry - target
        }
      ...
      const dateObj = new Date(trade.dateTime)
      ...
      if (dayIndex >= 0 && dayIndex <= 4) {
        ...
        if (typeof trade.riskReward === 'number' && !isNaN(trade.riskReward)) {
          weekdayTrades[dayOfWeek].totalRR += trade.riskReward
    ...
    const avgLoss =
      losingTrades > 0 ? Math.abs(totalLossAmountInternal / losingTrades) : 0
    const winRate = totalTrades > 0 ? (winningTrades / totalTrades) * 100 : 0

    const expectancy =
      (winRate / 100) * avgWin +
      ((100 - winRate) / 100) * (totalLossAmountInternal / losingTrades)
    ...
    for (let i = 0; i < sortedDates.length; i++) {
      ...
      if (pnl > 0) {
        ...
        maxConsecutiveWinDays = Math.max(
        ...
        maxConsecutiveLossDays = Math.max(
    ...
    const avgLossPerDay =
      dailyLossDays > 0 ? totalDailyLossPnl / dailyLossDays : 0
    const avgTradesPerDay = tradingDays > 0 ? totalTrades / tradingDays : 0
    const mostProfitableStrategy = Object.entries(strategyPerformance).reduce(
      (max, [strategy, data]) =>
        data.pnl > max.pnl ? { strategy, pnl: data.pnl } : max,
      { strategy: 'N/A', pnl: -Infinity },
    )
    const mostTradedSymbol = Object.entries(symbolPerformance).reduce(
      (max, [symbol, data]) =>
        data.count > max.count ? { symbol, count: data.count } : max,
      { symbol: 'N/A', count: 0 },
    )
    const mostProfitableSymbol = Object.entries(symbolPerformance).reduce(
      (max, [symbol, data]) =>
        data.pnl > max.pnl ? { symbol, pnl: data.pnl } : max,
      { symbol: 'N/A', pnl: -Infinity },
    )
    const leastProfitableSymbol = Object.entries(symbolPerformance).reduce(
      (min, [symbol, data]) =>
        data.pnl < min.pnl ? { symbol, pnl: data.pnl } : min,
      { symbol: 'N/A', pnl: Infinity },
    )
    const setupEffectiveness = Object.fromEntries(
      Object.entries(strategyPerformance).map(([strategy, data]) => [
        strategy,
        {
          ...
          winRate:
            data.totalTrades > 0 ? (data.wins / data.totalTrades) * 100 : 0,
        },
      ]),
    )
    ...
    const plannedRiskRewardRatio =
      plannedRRCount > 0 && totalPlannedRisk > 0
        ? totalPlannedReward / totalPlannedRisk
        : 0

    const realizedRiskRewardRatio =
      realizedRRCount > 0 ? totalRealizedRRSUM / realizedRRCount : 0
    const expectancyPerR = avgLoss !== 0 ? expectancy / avgLoss : 0
    ...
    setMetrics({
      ...
      dailyBreakEvenDays,
      mostProfitableDay:
        mostProfitableDay.pnl !== -Infinity
          ? {
              ...mostProfitableDay,
              date: new Date(mostProfitableDay.date).toLocaleDateString(
                'en-US',
                dateFormatOptions,
              ),
            }
          : { date: 'N/A', pnl: 0 },
      leastProfitableDay:
        leastProfitableDay.pnl !== Infinity
          ? {
              ...leastProfitableDay,
              date: new Date(leastProfitableDay.date).toLocaleDateString(
                'en-US',
                dateFormatOptions,
              ),
            }
          : { date: 'N/A', pnl: 0 },
      avgWinPerDay,
      avgLossPerDay: avgLossPerDay,
      avgCapitalUsedPerTrade:
        capitalUsedCount > 0 ? totalCapitalUsed / capitalUsedCount : 0,
      ...
      mostProfitableSymbol:
        mostProfitableSymbol.pnl !== -Infinity
          ? mostProfitableSymbol
          : { symbol: 'N/A', pnl: 0 },
      leastProfitableSymbol:
        leastProfitableSymbol.pnl !== Infinity
          ? leastProfitableSymbol
          : { symbol: 'N/A', pnl: 0 },
