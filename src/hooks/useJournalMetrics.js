...
export const useJournalMetrics = (tradeHistory) => {
  const [journalMetrics, setJournalMetrics] = useState({
    ...
    setupPerformance: [],
    overallSentiment: [], // CORRECTED: Initial state is now an empty array
  });
  ...
  const calculateJournalMetrics = useCallback(() => {
    if (!tradeHistory || tradeHistory.length === 0) {
      setJournalMetrics({
        ...
        mostUsedSetup: { setup: 'N/A', count: 0 },
        setupPerformance: [],
        overallSentiment: [], // CORRECTED: Now set to an empty array for empty data
      });
      return;
    }
    ...
    tradeHistory.forEach(trade => {
      ...
      const successes = trade.whatDidWellChecklist || [];
      const setup = trade.setup || 'Unknown';
      const emotionsBefore = trade.emotionsBefore;
      const emotionsAfter = trade.emotionsAfter;

      // Most Frequent Mistakes & Their P&L Impact
      ...
      // Most Frequent Successes & Their P&L Impact
      ...
      pnlBySetup[setup] = (pnlBySetup[setup] || { sum: 0, count: 0, wins: 0, losses: 0 });
      ...
      // Overall Sentiment
      if (emotionsBefore) {
          overallSentimentCounts[emotionsBefore] = (overallSentimentCounts[emotionsBefore] || 0) + 1;
      }
      if (emotionsAfter) {
          overallSentimentCounts[emotionsAfter] = (overallSentimentCounts[emotionsAfter] || 0) + 1;
      }
    });

    const totalTrades = tradeHistory.length;
    const totalSentimentEntries = tradeHistory.length * 2; // For before and after emotions

    const mostFrequentMistakes = Object.entries(mistakeCounts)
      .map(([label, count]) => ({ label, value: `${((count / totalTrades) * 100).toFixed(0)}%`, count }))
    ...
    const calculatedPnlByMistake = Object.entries(pnlByMistake)
      .map(([label, data]) => ({ label, value: data.sum / data.count }))
    ...
    const mostUsedSetup = Object.entries(setupCounts)
      .reduce((max, [setup, count]) => (count > max.count ? { setup, count } : max), { setup: 'N/A', count: 0 });
    ...
    const overallSentiment = Object.entries(overallSentimentCounts)
        .map(([label, count]) => ({ label, value: `${((count / totalSentimentEntries) * 100).toFixed(0)}%` }))
